...
public:
    int minSplitMerge(vector<int>& nums1, vector<int>& nums2) {
        ...
        vis.insert(nums1);
        while(q.empty() == false){
            auto [now, steps] = q.front();
            q.pop();
            int n = now.size();
            for(int i = 0; i<n; i++){
                for(int j = i; j<n; j++){
                    vector<int> split(now.begin()+i, now.begin()+j+1);
                    vector<int> remain;
                    remain.insert(remain.end(), now.begin(), now.begin()+i);
                    remain.insert(remain.end(), now.begin()+j+1, now.end());
                    for(int k = 0; k<=remain.size(); k++){
                        vector<int> temp = remain;
                        temp.insert(temp.begin()+k, split.begin(), split.end());
